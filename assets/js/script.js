...
const victoryText =
  document.querySelector("#victory-message"); /*get element winning message */
const startGameButton =
  document.querySelector("#new-game-button"); /*get new game button*/
...
startGameButton.addEventListener(
  "click",
  startGame
); /*new game when push on button*/

function startGame() {
  [gameNode, victoryText].forEach(
    (node) => (node.innerHTML = "")
  ); /*clean our board and vinning message when new game begins */
...
function generateArray(imageFilenames, cardAmount) {
  ...
  const elementCounts =
    {}; /*object. count how much times every element meets */
  ...
  while (randomArray.length < cardAmount) {
    ...
    if (elementCounts[randomElement] < 2) {
      /* each card can appear only twice */
